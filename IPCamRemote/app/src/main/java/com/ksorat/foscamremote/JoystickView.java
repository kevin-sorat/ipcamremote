...
public class JoystickView extends View {
	...
	private int prevUserX = Integer.MIN_VALUE, prevUserY = Integer.MIN_VALUE;
	...
	private void initJoystickView() {
		...
		bgPaint.setColor(Color.argb(125, 125, 125, 125));
		bgPaint.setStrokeWidth(1);
		bgPaint.setStyle(Paint.Style.FILL_AND_STROKE);

		handlePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		handlePaint.setColor(Color.rgb(20, 20, 150));
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		bgRadius = dimX / 2 - innerPadding - 20;
		handleRadius = (int) (d * 0.20);
		handleInnerBoundaries = handleRadius;
		movementRadius = Math.min(cX, cY) - handleInnerBoundaries - 20;
	}
	...
	private void constrainCircle() {
		float diffX = touchX;
		float diffY = touchY;
		double radial = java.lang.Math.sqrt((diffX * diffX) + (diffY * diffY));
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		// If public camera is not selected
	...
	private void reportOnMoved() {
		...
		if (moveListener != null) {
			...
			if (rx || ry) {
				...
				// Only perform OnMoved only if the userX and userY have changed, to improve performance
